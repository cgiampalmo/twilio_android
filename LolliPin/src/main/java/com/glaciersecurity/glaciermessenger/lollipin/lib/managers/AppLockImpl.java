...
public class AppLockImpl<T extends AppLockActivity> extends AppLock implements LifeCycleInterface {
    ...
    public String getSalt() {
        ...
        try {
            ...
            Cursor c = lollipinDB.rawQuery("SELECT * FROM " + TABLE_NAME + " where key = '" + PASSWORD_SALT_PREFERENCE_KEY + "';", null);
    ...
    public long getTimeout() {
        try {
            ...
            File dir = new File(root.getAbsolutePath() + "/" + STORAGE_DIRCTORY);
            SQLiteDatabase lollipinDB = SQLiteDatabase.openDatabase(dir + "/" + DATABASE_NAME, null, SQLiteDatabase.OPEN_READONLY);
            Cursor c = lollipinDB.rawQuery("SELECT * FROM " + TABLE_NAME + " where key = '" + TIMEOUT_MILLIS_PREFERENCE_KEY + "';", null);
            if ((c != null) && (c.getCount() > 0)) {
                c.moveToFirst();
                lollipinDB.close();
    ...
    public boolean pinChallengeCancelled() {
        try {
            ...
            File dir = new File(root.getAbsolutePath() + "/" + STORAGE_DIRCTORY);
            SQLiteDatabase lollipinDB = SQLiteDatabase.openDatabase(dir + "/" + DATABASE_NAME, null, SQLiteDatabase.OPEN_READONLY);
            Cursor c = lollipinDB.rawQuery("SELECT * FROM " + TABLE_NAME + " where key = '" + PIN_CHALLENGE_CANCELLED_PREFERENCE_KEY + "';", null);
            if ((c != null) && (c.getCount() > 0)) {
                c.moveToFirst();
    ...
    public boolean shouldShowForgot() {
        try {
            ...
            File dir = new File(root.getAbsolutePath() + "/" + STORAGE_DIRCTORY);
            SQLiteDatabase lollipinDB = SQLiteDatabase.openDatabase(dir + "/" + DATABASE_NAME, null, SQLiteDatabase.OPEN_READONLY);
            Cursor c = lollipinDB.rawQuery("SELECT * FROM " + TABLE_NAME + " where key = '" + SHOW_FORGOT_PREFERENCE_KEY + "';", null);
            if ((c != null) && (c.getCount() > 0)) {
                c.moveToFirst();
                boolean bvar = Boolean.parseBoolean(c.getString(c.getColumnIndex("value")));
                lollipinDB.close();
    ...
    public boolean onlyBackgroundTimeout() {
        try {
            ...
            File dir = new File(root.getAbsolutePath() + "/" + STORAGE_DIRCTORY);
            SQLiteDatabase lollipinDB = SQLiteDatabase.openDatabase(dir + "/" + DATABASE_NAME, null, SQLiteDatabase.OPEN_READONLY);
            Cursor c = lollipinDB.rawQuery("SELECT * FROM " + TABLE_NAME + " where key = '" + ONLY_BACKGROUND_TIMEOUT_PREFERENCE_KEY + "';", null);
            if ((c != null) && (c.getCount()) > 0) {
                c.moveToFirst();
                boolean bvar = Boolean.parseBoolean(c.getString(c.getColumnIndex("value")));
                lollipinDB.close();
    ...
    public long getLastActiveMillis() {
        try {
            ...
            File dir = new File(root.getAbsolutePath() + "/" + STORAGE_DIRCTORY);
            SQLiteDatabase lollipinDB = SQLiteDatabase.openDatabase(dir + "/" + DATABASE_NAME, null, SQLiteDatabase.OPEN_READONLY);
            Cursor c = lollipinDB.rawQuery("SELECT * FROM " + TABLE_NAME + " where key = '" + LAST_ACTIVE_MILLIS_PREFERENCE_KEY + "';", null);
            if ((c != null) && (c.getCount() > 0)) {
                c.moveToFirst();
                lollipinDB.close();
                return Long.parseLong(c.getString(c.getColumnIndex("value")));
    ...
    public boolean isFingerprintAuthEnabled() {
        try {
            ...
            File dir = new File(root.getAbsolutePath() + "/" + STORAGE_DIRCTORY);
            SQLiteDatabase lollipinDB = SQLiteDatabase.openDatabase(dir + "/" + DATABASE_NAME, null, SQLiteDatabase.OPEN_READONLY);
            Cursor c = lollipinDB.rawQuery("SELECT * FROM " + TABLE_NAME + " where key = '" + FINGERPRINT_AUTH_ENABLED_PREFERENCE_KEY + "';", null);
            if ((c != null) && (c.getCount() > 0)) {
                c.moveToFirst();
                boolean bvar = Boolean.parseBoolean(c.getString(c.getColumnIndex("value")));
                lollipinDB.close();
    ...
    public boolean checkPasscode(String passcode) {
        ...
        try {
            ...
            File dir = new File(root.getAbsolutePath() + "/" + STORAGE_DIRCTORY);
            SQLiteDatabase lollipinDB = SQLiteDatabase.openDatabase(dir + "/" + DATABASE_NAME, null, SQLiteDatabase.OPEN_READONLY);
            Cursor c = lollipinDB.rawQuery("SELECT * FROM " + TABLE_NAME + " where key = '" + PASSWORD_ALGORITHM_PREFERENCE_KEY + "';", null);
            if ((c != null) && (c.getCount() > 0)) {
                c.moveToFirst();
        ...
        try {
            ...
            // retrieve user specified key
            Cursor c = lollipinDB.rawQuery("SELECT * FROM " + TABLE_NAME + " where key = '" + PASSWORD_PREFERENCE_KEY + "';", null);
            if ((c != null) && (c.getCount() > 0)) {
                c.moveToFirst();
                storedPasscode = c.getString(c.getColumnIndex("value"));
            }

            // retrieve emergency key
            c = lollipinDB.rawQuery("SELECT * FROM " + TABLE_NAME + " where key = '" + EMERGENCY_PASSWORD_PREFERENCE_KEY + "';", null);
            if ((c != null) && (c.getCount() > 0)) {
                c.moveToFirst();
    ...
    public boolean setPasscode(String passcode) {
        ...
        if (passcode == null) {
            try {
                ...
                File dir = new File(root.getAbsolutePath() + "/" + STORAGE_DIRCTORY);
                SQLiteDatabase lollipinDB = SQLiteDatabase.openDatabase(dir + "/" + DATABASE_NAME, null, SQLiteDatabase.OPEN_READONLY);
                lollipinDB.rawQuery("DELETE FROM " + TABLE_NAME + " where key = '" + PASSWORD_PREFERENCE_KEY + "';", null);
                lollipinDB.rawQuery("DELETE FROM " + TABLE_NAME + " where key = '" + EMERGENCY_PASSWORD_PREFERENCE_KEY + "';", null);
                lollipinDB.close();
            } catch (SQLiteException e) {
                e.printStackTrace();
            }
            ...
            try {
                ...
                File dir = new File(root.getAbsolutePath() + "/" + STORAGE_DIRCTORY);
                SQLiteDatabase lollipinDB = SQLiteDatabase.openOrCreateDatabase(dir + "/" + DATABASE_NAME, null);
                lollipinDB.execSQL("CREATE TABLE IF NOT EXISTS Lollipin(key VARCHAR PRIMARY KEY,value VARCHAR);");
    ...
    private void printLollipinDB() {
        ...
        try {
            ...
            if ((c != null) && (c.getCount() > 0)) {
                c.moveToFirst();
                while (c.moveToNext()) {
                    Log.d("GOOBER", TABLE_NAME + ": " + c.getString(c.getColumnIndex("key")) + "::" + c.getString(c.getColumnIndex("value")));
                }
            } else {
