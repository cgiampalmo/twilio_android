...
public class CallParticipantsPage {
    ...
    public @NonNull
    List<TwilioCallParticipant> getCallParticipants() {
    ...
    public boolean equals(Object o) {
        ...
        boolean same = callParticipants.size() == that.callParticipants.size();
        if (same) {
            for (int i = 0; i < callParticipants.size(); i++) {
                if (!callParticipants.get(i).equals(that.callParticipants.get(i))) {
                    same = false;
                }
            }
        }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(callParticipants);
    }
}
