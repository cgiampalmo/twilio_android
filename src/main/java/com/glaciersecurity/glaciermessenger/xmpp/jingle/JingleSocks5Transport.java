...
import com.glaciersecurity.glaciermessenger.xmpp.jingle.stanzas.Content;
	...
	JingleSocks5Transport(JingleConnection jingleConnection, JingleCandidate candidate) {
		...
		try {
			...
			if (jingleConnection.getFtVersion() == Content.Version.FT_3) {
				Log.d(Config.LOGTAG, this.connection.getAccount().getLogJid() + ": using session Id instead of transport Id for proxy destination");
		...
		new Thread(() -> {
			...
			try {
				...
				if (fileInputStream == null) {
					Log.d(Config.LOGTAG, connection.getAccount().getLogJid() + ": could not create input stream");
		...
		new Thread(() -> {
			...
			try {
				...
				if (fileOutputStream == null) {
					callback.onFileTransferAborted();
					Log.d(Config.LOGTAG, connection.getAccount().getLogJid() + ": could not create output stream");
					return;
				}
				...
				while (remainingSize > 0) {
					...
					if (count == -1) {
						callback.onFileTransferAborted();
						Log.d(Config.LOGTAG, connection.getAccount().getLogJid() + ": file ended prematurely with " + remainingSize + " bytes remaining");
						return;
					} else {
